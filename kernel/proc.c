...
userinit(void)
{
  ...
  ukvmcopy(p->pagetable, p->kpagetable, 0, PGSIZE);
  p->sz = PGSIZE;
...
growproc(int n)
{
  ...
  if(n > 0){
	...
	ukvmcopy(p->pagetable, p->kpagetable, oldsz, sz);
  } else if(n < 0){
    sz = uvmdealloc(p->pagetable, sz, sz + n);
	ukvmdealloc(p->kpagetable, sz, sz + n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  if(ukvmcopy(np->pagetable, np->kpagetable, 0, p->sz) < 0) {
	freeproc(np);
	release(&np->lock);
	return -1;
  }
